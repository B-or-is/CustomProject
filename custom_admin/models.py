...
class Brand(models.Model):
    name = models.CharField(max_length=64, default="Superbrand")

    def __str__(self):
        return self.name


class Promo(models.Model):                          # модель "промоакция"
...
class Item(models.Model):               # абстрактная модель "предмет", от которой будут наследоваться другие модели
    description = models.TextField()
    ...
    promo = models.ManyToManyField(Promo, default=100)
...
class Dishwasher(Item):
    ...
    power = models.IntegerField(default=220)
    width = models.FloatField()
    height = models.FloatField()

    # функция для вывода указанным в ней цветом
